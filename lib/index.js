...
// Packages
// Aconst { Cookie } = require('tough-cookie')
// Aconst isUrl = require('is-url')
// Aconst useragentFromSeed = require('useragent-from-seed')
...
class Instagram {
  ...
  // Aheaders: {
  //  A referer: baseUrl + '/' + username + '/',
  //   A'x-instagram-gis': await this._getGis(`/${username}/`)
  // A }
  async getUserByUsername({ username }) {
    const user = await fetch(`https://www.instagram.com/${username}/?__a=1`)
    const data = await user.json()
    return data.graphql.user
    // Areturn this.request({
    //   uri: `//channel/?__a=1`,
    //   headers: {}
    // }).then(data => {
    //   console.log('DATA---------------')
